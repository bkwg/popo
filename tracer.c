...
static PopoStatus popo_attach_target(pid_t pid)
{
...
static PopoStatus popo_get_registers(pid_t pid, struct user_regs_struct* regs)
{
...
static PopoStatus popo_get_instruction(pid_t pid, struct user_regs_struct* regs, U64* insn)
{
...
static PopoStatus popo_single_step_tracee(pid_t tracee_pid)
{
...
static PopoStatus popo_do(pid_t tracee_pid)
{
    // TODO: actions performed shall be subjected to user choices

    struct user_regs_struct regs = { 0 };
    PopoStatus s = POPO_STATUS_FAILURE;
    
    //// 1. Get regs
    s = popo_get_registers(tracee_pid, &regs);
    if (POPO_FAILURE(s)) return POPO_STATUS_FAILURE;
    //
    // 2. Get insn
    U64 insn = 0;
    ...
    // 3. Taint and Disas
    popo_disas_and_taint(&regs, (char*)&insn);
...
static PopoStatus popo_handle_non_trap_signal(pid_t tracee_pid, siginfo_t* infop)
{
    // TODO: this check may be useless
    U64 last_element = POPO_ARRAY_LENGTH(signal_handler_msg_map) - 1;
...
static PopoStatus popo_handle_trap(pid_t tracee_pid)
{
    // TODO: actions performed shall be subjected to user choices
...
static PopoStatus popo_handle_tracee_signal(pid_t tracee_pid, siginfo_t *infop)
{
...
static PopoStatus popo_wait_for_signal(pid_t tracee_pid, siginfo_t* infop)
{
...
static PopoBool is_sigint_or_sigquit(siginfo_t* infop)
{
...
static PopoBool is_killed_exited_or_dumped(siginfo_t* infop)
{
 ...
 */
static PopoStatus popo_trace_tracee(pid_t tracee_pid)
{
    siginfo_t infop = { 0 };
    ...
    while ((s = POPO_SUCCESS(popo_wait_for_signal(tracee_pid, &infop)))
...
PopoStatus popo_spawn(PopoTracee* tracee, const char* image_path)
{
...
PopoStatus popo_attach(PopoTracee* tracee, pid_t pid)
{
    printf("[+] Attaching to process '%d'\n", pid);
    PopoStatus s = popo_attach_target(pid);
    if (POPO_FAILURE(s)) return POPO_STATUS_FAILURE;
